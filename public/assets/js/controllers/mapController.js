    ...
    function getPosition(position) {
        $http({
            ...
            $rootScope.map = new google.maps.Map(document.getElementById('map'), {
                zoom: 15,
                center: { lat: parseFloat($scope.currentUser.lat), lng: parseFloat($scope.currentUser.lng) },
            ...
            var marker = new google.maps.Marker({
                position: { lat: parseFloat($scope.currentUser.lat), lng: parseFloat($scope.currentUser.lng) },
                map: $rootScope.map,
